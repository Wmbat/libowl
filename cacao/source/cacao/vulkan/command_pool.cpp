...
namespace vkn
{
   ...
   builder::builder(const cacao::device& device, util::logger_wrapper logger) : m_logger{logger}
   {
      m_info.device = device.logical();
      m_info.queue_family_index = device.get_queue_index(cacao::queue_flag_bits::graphics);
   }
