...
namespace vkn
{
   ...
   auto device::select(selection_info&& info) -> result<device>
   {
      ...
      return find_suitable_device(
                info.available_devices,
                {.surface = std::move(info.surface), .p_logger = std::move(info.p_logger)})
   ...
   auto device::get_queue_index(queue_type type) const -> result<std::uint32_t>
   {
      return monad::try_wrap<vk::SystemError>([&] {
         ...
         .and_then([&](auto families) -> result<std::uint32_t> {
      ...
      -> result<device_data>
   {
      ...
      else
      {
         ...
         util::log_info(data.p_logger, "[vulkan] physical device selected: {}",
