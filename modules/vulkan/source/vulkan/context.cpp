...
namespace vulkan
{
   ...
   namespace detail
   {
      ...
      auto err_code(context_error err) -> error_t
      {
         return {{static_cast<int>(err), context_error_cat}};
      }
   } // namespace detail
      ...
      -> result<util::dynamic_array<vk::PhysicalDevice>>
   {
      return monad::try_wrap<vk::SystemError>([&] {
         ...
         .map_error([]([[maybe_unused]] auto err) {
            return detail::err_code(context_error::failed_to_enumerate_physical_devices);
         });
   }
   ...
   auto query_vulkan_version(context_data&& data) -> result<context_data>
   {
      return monad::try_wrap<vk::SystemError>([] {
         ...
         .map_error([&](vk::SystemError&& e) {
            ...
            return detail::err_code(context_error::failed_to_query_vulkan_version);
         })
         .and_then([&](std::uint32_t v) -> result<context_data> {
            ...
            {
               return monad::err(detail::err_code(context_error::vulkan_version_1_2_unavailable));
            }
         });
   }
   ...
   auto create_instance(context_data&& data) -> result<context_data>
   {
      ...
      {
         return monad::err(detail::err_code(context_error::window_extensions_not_present));
      }
      ...
      return monad::try_wrap<vk::SystemError>([&] {
         ...
         .map_error([&](vk::SystemError&& e) {
            ...
            return detail::err_code(context_error::failed_to_create_instance);
         })
      ...
      if constexpr (enable_validation_layers)
      {
         return monad::try_wrap<vk::SystemError>([&] {
            ...
            .map_error([]([[maybe_unused]] const auto& err) -> error_t {
               return detail::err_code(context_error::failed_to_create_debug_utils);
            })
