...
public:
   ...
   template <typename T, typename = std::enable_if_t<!std::is_same<delegate, std::decay_t<T>>::value>>
   delegate( T&& f ) :
   ...
   template <typename T, typename = std::enable_if<!std::is_same<delegate, std::decay_t<T>>::value>>
...
private:
   ...
   template <typename T>
   static typename ::std::enable_if<!( is_member_pair<T>::value || is_const_member_pair<T>::value ), R>::type functor_stub(
