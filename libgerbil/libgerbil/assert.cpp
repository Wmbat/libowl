...
namespace assert_detail
{
                                            ...
                                            const char* message, source_location location)
   {
      if (!c)
      {
         ...
         if (message == nullptr)
         {
            fmt::print(stderr, "{} failed at {}:{}: {}\n", action, location.file, location.line,
                       location.function);
         }
         else
         {
...
#   endif
   ...
   {
      auto basic_type =
         get_info<BasicType, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_BASETYPE>(type_index, proc, modbase);
      // auto length = get_info<ULONG64, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_LENGTH>(type_index, proc,
      // modbase);
      switch (basic_type)
      {
   ...
   {
      auto tag =
         get_info<SymTagEnum, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_SYMTAG>(type_index, proc, modbase);
      switch (tag)
      {
      ...
      for (int i = 0; i < frames; i++)
      {
         ...
         if (SymFromAddr(proc, (DWORD64)addrs[i], &displacement.a, symbol))
         {
            if (got_line)
            {
               ...
               // There's a phenomina with DIA not inserting commas after template parameters. Fix
               // them here.
...
#   endif
   ...
   [[gnu::cold]] auto union_regexes(std::initializer_list<std::string_view> regexes) -> std::string
   {
   ...
   private:
      [[gnu::cold]] analysis()
      {
         ...
         *std::find(std::begin(punctuators), std::end(punctuators), "<:") += "(?!:[^:>])";
         ...
         std::string fractional_constant = stringf("(?:(?:%s)?\\.%s|%s\\.)", digit_sequence.c_str(),
                                                   digit_sequence.c_str(), digit_sequence.c_str());
   ...
   public:
      ...
      [[gnu::cold]] auto _highlight(const std::string& expression) -> std::vector<highlight_block>
      try
      {
         ...
         for (size_t i = 0; i < tokens.size(); i++)
         {
            ...
            switch (token.token_type)
            {
               ...
               case token_e::number:
                  ...
                  break;
               case token_e::string:
                  output.emplace_back(GREEN,
                                      std::regex_replace(token.str, escapes_re, BLUE "$&" GREEN));
                  break;
      ...
      [[gnu::cold]] auto find_last_non_ws(const std::vector<token_t>& tokens, size_t i) -> token_t
      {
   ...
   [[gnu::cold]] static constexpr auto log10(int n) -> int
   {
      ...
      for (int i = 0;
           ...
           [] {
              int j = 0,
                  n =
                     std::numeric_limits<int>::max(); // note: `j` used instead of `i` because of
   ...
   [[gnu::cold]] void print_stacktrace()
   {
      if (auto _trace = get_stacktrace())
      {
         ...
         // Two boundaries: main is found here, assert_detail stuff is cut off during stack trace
         // generation
         ...
         const auto max_line_number_width =
            size_t(log10(std::max_element(trace.begin(), trace.begin() + std::ptrdiff_t(end) + 1,
                                          [](const assert_detail::stacktrace_entry& a,
                                             const assert_detail::stacktrace_entry& b) {
