...
namespace collision
{
   ...
   system::system(create_info&& info) : mp_registry{info.p_registry.get()} {}
   ...
   void
   system::resolve_particle_to_box_collision(std::chrono::duration<float, std::milli> time_step)
   {
      auto view = mp_registry->view<render::component::transform, sph::component::particle>();

      for (auto entity : view)
      {
         auto& transform = view.get<render::component::transform>(entity);
         auto& particle = view.get<sph::component::particle>(entity);

         const float t0 = -time_step.count();
         const float t1 = 0.0f;

         const auto p0 = transform.position + t0 * particle.velocity;
         const auto p1 = transform.position + t1 * particle.velocity;
         ...
         for (auto entity : view)
         {
            ...
            if (distance - particle.radius < collision::epsilon) // Handle collision
            {
               ...
               if (distance0 > distance)
               {
                  ...
                  p = sphere_t1.center + normal * (distance - particle.radius);
               }
               else
               {
