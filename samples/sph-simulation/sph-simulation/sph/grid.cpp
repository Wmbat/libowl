...
namespace sph
{
   ...
   void grid::update_layout(std::span<particle> particles)
   {
      ...
      for (auto& p : particles)
      {
         ...
         m_cells.at(static_cast<std::size_t>(offset)).particles.push_back(&p);
      }
   }

   auto grid::cells() -> std::span<cell> { return m_cells; }

   auto grid::lookup_neighbours(const glm::i64vec3& grid_pos) -> std::vector<particle*>
   {
      ...
      for (int x : vi::ints(early_x, late_x))
      {
         for (int y : vi::ints(early_y, late_y))
         {
            for (int z : vi::ints(early_z, late_z))
            {
               ...
               auto& c = m_cells.at(static_cast<std::size_t>(offset));
